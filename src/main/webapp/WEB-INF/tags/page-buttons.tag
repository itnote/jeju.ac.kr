...
<div data-page-buttons="">
    <div class="button-warp">
        ...
        <c:if test="${authGroupMenu.delAh eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="del"><i class="cqc-minus"></i> <%=MessageUtils.getMessage(request, "ax.admin.delete")%></button>
        </c:if>
        ...
        <c:if test="${authGroupMenu.fn2Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn2">${function2Label}</button>
        </c:if>

        <c:if test="${authGroupMenu.fn3Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn3">${function3Label}</button>
        </c:if>

        <c:if test="${authGroupMenu.fn4Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn4">${function4Label}</button>
        </c:if>
