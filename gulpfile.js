...
gulp.task('axboot-initializr-deploy', function () {
    ...
    gulp.src([
        ...
        .pipe(replace(/import=\"com\.chequer\.axboot\.admin/g, 'import="com.infomind.axboot'))
        .pipe(gulp.dest('ax-boot-initialzr/src/main/resources/templates/webapp'));
    ...
    gulp.src([
        ...
        .pipe(replace(/com\.chequer\.axboot\.admin/g, 'com.infomind.axboot'))
...
// 일반 컴파일
gulp.task('ax5calendar', function () {
    return gulp.src(ASSETS_SRC+ '/plugins/ax5ui-calendar/src/ax5calendar.scss')  // 입력 경로
        .pipe(plumber({errorHandler: errorAlert}))
        .pipe(sass({outputStyle: 'compressed'}))
        .pipe(gulp.dest(ASSETS_SRC+ '/plugins/ax5ui-calendar/dist/'));  // 출력 경로
});
